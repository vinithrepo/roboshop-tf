...
module "app" {
  ...
  vpc_id                  = local.vpc_id
  private_alb_name  = lookup(lookup(lookup(module.alb, each.value["lb_type"],  null), "alb", null), "dns_name", null)
  private_listener  = lookup(lookup(lookup(module.alb, each.value["lb_type"],  null), "listener", null), "arn" , null)
}
